...
public class HomeController {
    ...
    private void createAlbums() {
        TransactionStatus albumTransaction = albumsTransactionManager.getTransaction(null);
        for (Album album : albumFixtures.load()) {
            albumsBean.addAlbum(album);
        }

        albumsTransactionManager.commit(albumTransaction);
    }

    private void createMovies() {
        TransactionStatus movieTransaction = moviesTransactionManager.getTransaction(null);

        for (Movie movie : movieFixtures.load()) {
            moviesBean.addMovie(movie);
        }

        moviesTransactionManager.commit(movieTransaction);
    }
}
